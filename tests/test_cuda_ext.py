"""Tests for cuBLASLt C++ extension fused dual FP8 GEMM."""
from __future__ import annotations

import math

import pytest
import torch
import torch.nn as nn

from tcfp.core import TCFPMode, to_fp8_e4m3

# Skip entire module if no CUDA
pytestmark = [
    pytest.mark.skipif(
        not torch.cuda.is_available(), reason="CUDA not available"
    ),
]

DEVICE = "cuda"

_HAS_CUDA_EXT = False
try:
    from tcfp.cuda_ext import (
        cuda_ext_dual_gemm_forward,
        is_cuda_ext_available,
    )

    _HAS_CUDA_EXT = is_cuda_ext_available()
except ImportError:
    pass

requires_cuda_ext = pytest.mark.skipif(
    not _HAS_CUDA_EXT, reason="cuBLASLt C++ extension not available"
)


# -- Helpers -----------------------------------------------------------------


def _reference_dual_gemm_forward(
    act_fp8: torch.Tensor,
    w_hi_fp8: torch.Tensor,
    w_lo_fp8: torch.Tensor,
    act_inv: torch.Tensor,
    w_hi_inv: torch.Tensor,
    w_lo_inv: torch.Tensor,
) -> torch.Tensor:
    """Reference: two torch._scaled_mm calls + add."""
    hi = torch._scaled_mm(
        act_fp8,
        w_hi_fp8.t(),
        scale_a=act_inv,
        scale_b=w_hi_inv,
        out_dtype=torch.float32,
        use_fast_accum=False,
    )
    lo = torch._scaled_mm(
        act_fp8,
        w_lo_fp8.t(),
        scale_a=act_inv,
        scale_b=w_lo_inv,
        out_dtype=torch.float32,
        use_fast_accum=False,
    )
    return hi + lo


def _make_fp8_pair(
    shape: tuple[int, ...],
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    """Create FP8 hi/lo weight decomposition from random data."""
    w = torch.randn(shape, device=DEVICE)
    w_hi_fp8, w_hi_inv = to_fp8_e4m3(w)
    residual = w - w_hi_fp8.float() * w_hi_inv
    w_lo_fp8, w_lo_inv = to_fp8_e4m3(residual)
    return w_hi_fp8, w_hi_inv, w_lo_fp8, w_lo_inv


# -- Forward Correctness ----------------------------------------------------


@requires_cuda_ext
class TestCudaExtForwardCorrectness:
    """cuBLASLt forward matches two-_scaled_mm reference."""

    @pytest.mark.parametrize(
        "M, K, N",
        [(64, 128, 64), (256, 512, 256), (128, 256, 512)],
        ids=["small", "medium", "non-square"],
    )
    def test_matches_reference(self, M: int, K: int, N: int) -> None:
        torch.manual_seed(42)
        act = torch.randn(M, K, device=DEVICE)
        act_fp8, act_inv = to_fp8_e4m3(act)
        w_hi_fp8, w_hi_inv, w_lo_fp8, w_lo_inv = _make_fp8_pair((N, K))

        result = cuda_ext_dual_gemm_forward(  # pyright: ignore[reportPossiblyUnboundVariable]
            act_fp8, w_hi_fp8, w_lo_fp8, act_inv, w_hi_inv, w_lo_inv
        )
        ref = _reference_dual_gemm_forward(
            act_fp8, w_hi_fp8, w_lo_fp8, act_inv, w_hi_inv, w_lo_inv
        )

        assert result.shape == ref.shape == (M, N)
        torch.testing.assert_close(result, ref, atol=1e-2, rtol=1e-2)

    def test_large_matrices(self) -> None:
        """cuBLASLt works at large dimensions."""
        torch.manual_seed(42)
        M, K, N = 1024, 1024, 1024
        act = torch.randn(M, K, device=DEVICE)
        act_fp8, act_inv = to_fp8_e4m3(act)
        w_hi_fp8, w_hi_inv, w_lo_fp8, w_lo_inv = _make_fp8_pair((N, K))

        result = cuda_ext_dual_gemm_forward(  # pyright: ignore[reportPossiblyUnboundVariable]
            act_fp8, w_hi_fp8, w_lo_fp8, act_inv, w_hi_inv, w_lo_inv
        )
        ref = _reference_dual_gemm_forward(
            act_fp8, w_hi_fp8, w_lo_fp8, act_inv, w_hi_inv, w_lo_inv
        )
        torch.testing.assert_close(result, ref, atol=1e-2, rtol=1e-2)

    def test_zero_lo_matches_single_gemm(self) -> None:
        """When W_lo is zero, output = single GEMM from W_hi."""
        torch.manual_seed(42)
        M, K, N = 64, 128, 64
        act = torch.randn(M, K, device=DEVICE)
        act_fp8, act_inv = to_fp8_e4m3(act)
        w = torch.randn(N, K, device=DEVICE)
        w_hi_fp8, w_hi_inv = to_fp8_e4m3(w)

        # Zero residual
        w_lo_fp8 = torch.zeros(N, K, device=DEVICE).to(torch.float8_e4m3fn)
        w_lo_inv = torch.ones(1, device=DEVICE, dtype=torch.float32)

        result = cuda_ext_dual_gemm_forward(  # pyright: ignore[reportPossiblyUnboundVariable]
            act_fp8, w_hi_fp8, w_lo_fp8, act_inv, w_hi_inv, w_lo_inv
        )
        single = torch._scaled_mm(
            act_fp8,
            w_hi_fp8.t(),
            scale_a=act_inv,
            scale_b=w_hi_inv,
            out_dtype=torch.float32,
            use_fast_accum=False,
        )
        torch.testing.assert_close(result, single, atol=1e-4, rtol=1e-4)

    def test_output_dtype_float32(self) -> None:
        """Output is FP32."""
        M, K, N = 64, 64, 64
        act = torch.randn(M, K, device=DEVICE)
        act_fp8, act_inv = to_fp8_e4m3(act)
        w_hi_fp8, w_hi_inv, w_lo_fp8, w_lo_inv = _make_fp8_pair((N, K))

        result = cuda_ext_dual_gemm_forward(  # pyright: ignore[reportPossiblyUnboundVariable]
            act_fp8, w_hi_fp8, w_lo_fp8, act_inv, w_hi_inv, w_lo_inv
        )
        assert result.dtype == torch.float32

    def test_all_finite(self) -> None:
        """No NaN or Inf in output for typical inputs."""
        torch.manual_seed(123)
        M, K, N = 256, 256, 256
        act = torch.randn(M, K, device=DEVICE)
        act_fp8, act_inv = to_fp8_e4m3(act)
        w_hi_fp8, w_hi_inv, w_lo_fp8, w_lo_inv = _make_fp8_pair((N, K))

        result = cuda_ext_dual_gemm_forward(  # pyright: ignore[reportPossiblyUnboundVariable]
            act_fp8, w_hi_fp8, w_lo_fp8, act_inv, w_hi_inv, w_lo_inv
        )
        assert torch.isfinite(result).all()


    # NOTE: cuBLASLt backward_dx is not tested here because cuBLASLt
    # doesn't support E5M2 as A-type on SM 12.0+ (Blackwell).
    # Backward dX falls back to Triton or 2x _scaled_mm automatically.


# -- Integration Tests -------------------------------------------------------


@requires_cuda_ext
class TestCudaExtIntegration:
    """End-to-end tests with TCFPLinear (cuBLASLt forward, fallback backward)."""

    def test_tcfp12_linear_fwd_bwd(self) -> None:
        """TCFPLinear with cuBLASLt forward + fallback backward."""
        from tcfp.nn import TCFPLinear

        layer = TCFPLinear(
            128,
            64,
            mode=TCFPMode.TCFP12,
            use_tensor_cores=True,
            use_cuda_ext=True,
            use_fused_kernel=False,
        ).to(DEVICE)

        x = torch.randn(8, 128, device=DEVICE, requires_grad=True)
        out = layer(x)
        assert out.shape == (8, 64)

        loss = out.sum()
        loss.backward()
        assert x.grad is not None
        assert x.grad.shape == (8, 128)
        assert torch.isfinite(x.grad).all()

    def test_fwd_bwd_3d_input(self) -> None:
        """3D input (batch, seq, dim) works with cuBLASLt."""
        from tcfp.nn import TCFPLinear

        layer = TCFPLinear(
            256,
            128,
            mode=TCFPMode.TCFP12,
            use_tensor_cores=True,
            use_cuda_ext=True,
            use_fused_kernel=False,
        ).to(DEVICE)

        x = torch.randn(4, 16, 256, device=DEVICE, requires_grad=True)
        out = layer(x)
        assert out.shape == (4, 16, 128)

        loss = out.sum()
        loss.backward()
        assert x.grad is not None
        assert x.grad.shape == (4, 16, 256)

    def test_error_feedback_works(self) -> None:
        """Error feedback state works with cuBLASLt dispatch."""
        from tcfp.nn import TCFPLinear

        layer = TCFPLinear(
            128,
            64,
            mode=TCFPMode.TCFP12,
            use_tensor_cores=True,
            error_feedback=True,
            use_cuda_ext=True,
            use_fused_kernel=False,
        ).to(DEVICE)

        x = torch.randn(8, 128, device=DEVICE)
        # Run two forward passes -- error feedback should accumulate
        out1 = layer(x)
        out1.sum().backward()
        out2 = layer(x)
        out2.sum().backward()
        # Error state should be populated
        assert layer._error_state is not None
        assert len(layer._error_state._buffers) > 0

    def test_convert_to_tcfp_passes_cuda_ext(self) -> None:
        """convert_to_tcfp forwards use_cuda_ext parameter."""
        from tcfp.nn import TCFPLinear, convert_to_tcfp

        model = nn.Sequential(
            nn.Linear(128, 64),
            nn.ReLU(),
            nn.Linear(64, 32),
        ).to(DEVICE)
        convert_to_tcfp(
            model,
            mode=TCFPMode.TCFP12,
            use_tensor_cores=True,
            use_cuda_ext=True,
        )
        for m in model.modules():
            if isinstance(m, TCFPLinear):
                assert m.use_cuda_ext == _HAS_CUDA_EXT

    def test_training_convergence(self) -> None:
        """Loss decreases over training with cuBLASLt dispatch."""
        from tcfp.nn import convert_to_tcfp

        torch.manual_seed(42)
        x = torch.randn(32, 128, device=DEVICE)
        target = torch.randn(32, 32, device=DEVICE)

        model = nn.Sequential(
            nn.Linear(128, 64),
            nn.ReLU(),
            nn.Linear(64, 32),
        ).to(DEVICE)
        convert_to_tcfp(
            model,
            mode=TCFPMode.TCFP12,
            use_tensor_cores=True,
            use_cuda_ext=True,
            use_fused_kernel=False,
        )
        opt = torch.optim.Adam(model.parameters(), lr=1e-3)

        losses = []
        for _ in range(50):
            out = model(x)
            loss = nn.functional.mse_loss(out, target)
            opt.zero_grad()
            loss.backward()
            opt.step()
            losses.append(loss.item())

        early = sum(losses[:5]) / 5
        late = sum(losses[-5:]) / 5
        assert all(math.isfinite(v) for v in losses), "NaN or Inf"
        assert late < early, (
            f"Loss did not decrease: {early:.4f} -> {late:.4f}"
        )

    def test_numerical_match_vs_triton(self) -> None:
        """cuBLASLt matches Triton fused kernel output."""
        try:
            from tcfp.kernels import (
                fused_dual_gemm_forward,
                is_triton_available,
            )

            if not is_triton_available():
                pytest.skip("Triton not available for comparison")
        except ImportError:
            pytest.skip("Triton not available for comparison")

        torch.manual_seed(42)
        M, K, N = 256, 256, 256
        act = torch.randn(M, K, device=DEVICE)
        act_fp8, act_inv = to_fp8_e4m3(act)
        w_hi_fp8, w_hi_inv, w_lo_fp8, w_lo_inv = _make_fp8_pair((N, K))

        triton_out = fused_dual_gemm_forward(
            act_fp8, w_hi_fp8, w_lo_fp8, act_inv, w_hi_inv, w_lo_inv
        )
        cuda_out = cuda_ext_dual_gemm_forward(  # pyright: ignore[reportPossiblyUnboundVariable]
            act_fp8, w_hi_fp8, w_lo_fp8, act_inv, w_hi_inv, w_lo_inv
        )
        torch.testing.assert_close(cuda_out, triton_out, atol=1e-2, rtol=1e-2)


# -- Edge Cases --------------------------------------------------------------


@requires_cuda_ext
class TestCudaExtEdgeCases:
    """Edge cases for dimension alignment and scale values."""

    def test_minimum_dimensions(self) -> None:
        """Works with minimum 16x16 matrices."""
        M, K, N = 16, 16, 16
        act = torch.randn(M, K, device=DEVICE)
        act_fp8, act_inv = to_fp8_e4m3(act)
        w_hi_fp8, w_hi_inv, w_lo_fp8, w_lo_inv = _make_fp8_pair((N, K))

        result = cuda_ext_dual_gemm_forward(  # pyright: ignore[reportPossiblyUnboundVariable]
            act_fp8, w_hi_fp8, w_lo_fp8, act_inv, w_hi_inv, w_lo_inv
        )
        assert result.shape == (M, N)
        assert torch.isfinite(result).all()

    def test_non_power_of_2_dims(self) -> None:
        """Works with dimensions not powers of 2 (but divisible by 16)."""
        M, K, N = 48, 80, 112
        act = torch.randn(M, K, device=DEVICE)
        act_fp8, act_inv = to_fp8_e4m3(act)
        w_hi_fp8, w_hi_inv, w_lo_fp8, w_lo_inv = _make_fp8_pair((N, K))

        result = cuda_ext_dual_gemm_forward(  # pyright: ignore[reportPossiblyUnboundVariable]
            act_fp8, w_hi_fp8, w_lo_fp8, act_inv, w_hi_inv, w_lo_inv
        )
        ref = _reference_dual_gemm_forward(
            act_fp8, w_hi_fp8, w_lo_fp8, act_inv, w_hi_inv, w_lo_inv
        )
        torch.testing.assert_close(result, ref, atol=1e-2, rtol=1e-2)

    def test_extra_repr_shows_cuda_ext(self) -> None:
        """extra_repr includes use_cuda_ext when active."""
        from tcfp.nn import TCFPLinear

        layer = TCFPLinear(
            128,
            64,
            mode=TCFPMode.TCFP12,
            use_tensor_cores=True,
            use_cuda_ext=True,
        )
        r = layer.extra_repr()
        if _HAS_CUDA_EXT:
            assert "use_cuda_ext=True" in r
        else:
            assert "use_cuda_ext" not in r


# -- Fallback Tests ----------------------------------------------------------


class TestCudaExtFallback:
    """Tests that the system works without cuBLASLt extension."""

    def test_tcfp12_without_cuda_ext(self) -> None:
        """TCFPLinear works when use_cuda_ext=False."""
        from tcfp.nn import TCFPLinear

        layer = TCFPLinear(
            128,
            64,
            mode=TCFPMode.TCFP12,
            use_tensor_cores=True,
            use_cuda_ext=False,
        ).to(DEVICE)

        x = torch.randn(8, 128, device=DEVICE, requires_grad=True)
        out = layer(x)
        assert out.shape == (8, 64)
        loss = out.sum()
        loss.backward()
        assert x.grad is not None
